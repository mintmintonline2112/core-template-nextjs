 ...
 *
 * Biến `--k` (hệ số thu nhỏ hạt, 1 → 0.45) đặt bằng inline style vì giá trị
 * tính theo vị trí từng mục; class Tailwind chỉ đọc lại qua calc().
 *
...
export function SizeRuler({ sizes, note }: { sizes: string[]; note?: string }) {
  ...
  return (
    <div
      className={`${PANEL_BG} rounded-lg px-[clamp(1.2rem,3vw,2.4rem)] pt-[clamp(1.8rem,4vw,2.6rem)] pb-5 shadow-lift`}
    >
      <ol className="flex items-end justify-between gap-2 border-b border-gold-300/40 max-[760px]:flex-wrap max-[760px]:justify-center max-[760px]:gap-x-7 max-[760px]:gap-y-5 max-[760px]:border-b-0">
        {sizes.map((size, index) => (
          <li
            key={size}
            className='flex min-w-0 flex-col items-center gap-2 after:mt-2 after:h-[10px] after:w-px after:bg-gold-300/70 after:content-[""] max-[760px]:after:hidden'
          ...
          >
            ...
            <span className="font-display text-h5 leading-none font-semibold text-light">
              {size}
            </span>
      ...
      {note ? (
        <p className="mt-4 mb-0 text-center font-display text-base text-gold-300 italic">
          {note}
        </p>
      ) : null}
    </div>
