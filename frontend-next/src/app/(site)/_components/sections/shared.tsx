...
export const meta = (section?: PageSection | null): Meta => (section?.metadata ?? {}) as Meta;
...
export function str(section: PageSection | null | undefined, key: string): string | undefined {
  const value = meta(section)[key];
...
export function strings(section: PageSection | null | undefined, key: string): string[] {
 ...
 */
export function items(section: PageSection | null | undefined, key: string): Array<Record<string, string>> {
  const raw = meta(section)[key];
  if (!Array.isArray(raw)) return [];
  return raw
    .map((item) => {
      if (typeof item === "string") return item.trim() ? { label: item.trim(), title: item.trim() } : null;
...
export function layoutOf<L extends string>(
  ...
  return raw && (allowed as readonly string[]).includes(raw) ? (raw as L) : fallback;
}

/** id neo trên trang = section key; thiếu section thì dùng tên component. */
export const anchorId = (section: PageSection | null | undefined, fallback: string) =>
...
export const ICONS: Record<string, ReactNode> = {
  ...
  leaf: <path d="M12 2c1 4-1 7-4 9 4 0 7-2 8-6 .8 5-2 10-8 11C4 15 4 8 8 5c-1 3 0 5 1 6 0-4 1-7 3-9z" />,
  scale: <><path d="M12 4v16M5 8l7-4 7 4" /><path d="M5 8l-2.5 6a3.5 3.5 0 0 0 7 0L7 8M19 8l-2.5 6a3.5 3.5 0 0 0 7 0L21 8" /><path d="M8 20h8" /></>,
  tune: <><path d="M4 21v-4M4 13v-2M4 7V3M12 21v-8M12 9V3M20 21v-2M20 15V3" /><circle cx="4" cy="15" r="2" /><circle cx="12" cy="11" r="2" /><circle cx="20" cy="17" r="2" /></>,
  ship: <path d="M3 17h18l-2 4H5l-2-4zM6 17V9l4-2v10M14 17V7l4 2v8" />,
  heart: <path d="M12 21c-5-3.5-8-7-8-11a4.5 4.5 0 0 1 8-2.8A4.5 4.5 0 0 1 20 10c0 4-3 7.5-8 11z" />,
  almond: <><path d="M12 3.5 C 16 7.5 18.5 12 17.2 15.9 C 16.2 18.9 7.8 18.9 6.8 15.9 C 5.5 12 8 7.5 12 3.5 Z" /><path d="M12 7.2 C 13.8 9.6 14.9 12.1 14.3 14.5" /></>,
  product: <><path d="M12 3c3.5 3.6 5.6 7.6 4.5 11-1 3-8 3-9 0C6.4 10.6 8.5 6.6 12 3z" /><path d="M12 7c1.6 2.2 2.4 4.4 2 6.4" /></>,
  ruler: <><rect x="2.5" y="8" width="19" height="8" rx="1" /><path d="M6.5 8v3M10.5 8v4M14.5 8v3M18.5 8v4" /></>,
  grade: <><circle cx="12" cy="10" r="6.5" /><path d="m9.2 10 2 2 3.6-3.8M8.5 15.5 7 21l5-2.5 5 2.5-1.5-5.5" /></>,
  calendar: <><rect x="3.5" y="5" width="17" height="15" rx="1.5" /><path d="M3.5 10h17M8 3v4M16 3v4" /></>,
  box: <><path d="M12 3 3.5 7v10L12 21l8.5-4V7L12 3z" /><path d="M3.5 7 12 11l8.5-4M12 11v10" /></>,
  pallet: <><rect x="2.5" y="7" width="19" height="11" rx="1" /><path d="M6.5 7v11M10.5 7v11M14.5 7v11M18.5 7v11" /></>,
  building: <><path d="M4 21V8l8-5 8 5v13" /><path d="M8 21v-8h8v8M3 21h18" /></>,
  mail: <><rect x="3" y="5" width="18" height="14" rx="2" /><path d="m3.5 6.5 8.5 7 8.5-7" /></>,
  phone: <path d="M5 4h4l1.8 4.5-2.3 1.7a13 13 0 0 0 5.3 5.3l1.7-2.3L20 15v4a1.5 1.5 0 0 1-1.6 1.5C10.8 20 4 13.2 3.5 5.6A1.5 1.5 0 0 1 5 4z" />,
  blanched: <><path d="M12 3.5 C 16 7.5 18.5 12 17.2 15.9 C 16.2 18.9 7.8 18.9 6.8 15.9 C 5.5 12 8 7.5 12 3.5 Z" /><path d="M19.5 4.5v3M18 6h3" /></>,
  sliced: <><ellipse cx="9" cy="12" rx="4" ry="8" /><ellipse cx="15" cy="12" rx="4" ry="8" opacity="0.55" /></>,
  slivered: <path d="M6 20 9 4M11.5 20 14.5 4M17 20 20 4" />,
  diced: <><rect x="4" y="4" width="6.5" height="6.5" rx="1" /><rect x="13.5" y="4" width="6.5" height="6.5" rx="1" /><rect x="4" y="13.5" width="6.5" height="6.5" rx="1" /><rect x="13.5" y="13.5" width="6.5" height="6.5" rx="1" /></>,
  flour: <><path d="M4 19c0-5 3.6-8.5 8-8.5s8 3.5 8 8.5H4z" /><circle cx="9" cy="6" r="0.4" /><circle cx="13.5" cy="4.5" r="0.4" /><circle cx="16.5" cy="7.5" r="0.4" /><circle cx="12" cy="8" r="0.4" /></>,
};

/** Thứ tự xoay vòng khi item không đặt tên icon. */
export const ICON_CYCLE = ["pin", "leaf", "scale", "tune", "ship", "heart"] as const;
...
export function IconBadge({ children, className = "icon-badge" }: { children: ReactNode; className?: string }) {
  return (
...
export function InlineNote({
  ...
  if (!text) return null;
  return <p className={className} style={style} dangerouslySetInnerHTML={{ __html: inlineHtml(text) }} />;
}
...
export function Head({
  ...
  if (!section?.heading && !section?.subheading && !(intro && section?.content)) return null;
  return (
...
/** Giá trị thống kê: chữ số hiển thị lớn, ký tự ′ / + thu nhỏ (stat-suffix); số ở đầu tự chạy đếm. */
...
/** Dải số liệu (dùng trong hero và section stats). */
export function StatsList({ stats, className }: { stats: Array<Record<string, string>>; className: string }) {
